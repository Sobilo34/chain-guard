...
/** CRE workflow sub-steps shown in Full Analysis modal (pipeline + streamed to log). */
const CRE_SUB_STEP_LABELS = [
  "Fetching contract state…",
  "Fetching market data (price feeds)…",
  "Running AI risk analysis…",
  "Building report…",
  "Writing report on-chain…",
];
...
export default function ContractDetailPage({
  ...
  const [analysisFallbackToOnchain, setAnalysisFallbackToOnchain] =
    useState(false);
  const [riskEngineSubStep, setRiskEngineSubStep] = useState(0);
  const [creWaitingStartTime, setCreWaitingStartTime] = useState<number | null>(
    null,
  );
  ...
  const [needMoreInfoFormValues, setNeedMoreInfoFormValues] = useState<
  ...
  const chainSelectorName = data
    ? getChainSelectorNameFromContract(data)
    : "ethereum-mainnet";
  const {
  ...
  // Stream simulation steps into the log when each sub-step advances (so user sees CRE workflow in the console).
  const riskEngineSubStepLoggedRef = useRef(-1);
  useEffect(() => {
    if (
      analysisStage !== "onchain_requested" ||
      !lastRequestId ||
      ...
      riskEngineSubStepLoggedRef.current = -1;
      return;
    }
    if (riskEngineSubStep <= riskEngineSubStepLoggedRef.current) return;
    riskEngineSubStepLoggedRef.current = riskEngineSubStep;
    // Log the step we just started (index 0 when we advance to 1, etc.)
    if (riskEngineSubStep >= 1) {
      const label = CRE_SUB_STEP_LABELS[riskEngineSubStep - 1];
      if (label) setAnalysisLogLines((prev) => [...prev, label]);
    }
    if (riskEngineSubStep === 4) {
      setAnalysisLogLines((prev) => [...prev, CRE_SUB_STEP_LABELS[4]]);
    }
  }, [analysisStage, lastRequestId, creAssessment, riskEngineSubStep]);

  // Update elapsed time every second while waiting for CRE result.
  useEffect(() => {
    if (creWaitingStartTime === null || creAssessment) return;
    const t = setInterval(() => {
      setCreWaitingElapsedSec(
        Math.floor((Date.now() - creWaitingStartTime) / 1000),
      );
    }, 1000);
    return () => clearInterval(t);
  ...
  useEffect(() => {
    if (analysisStage !== "onchain_requested")
  ...
  useEffect(() => {
    if (creRequestSuccess && lastRequestId && !analysisFallbackToOnchain) {
      toast.success("Request submitted onchain", {
        description:
          "CRE workflow will process and write the result onchain. You can poll for the result.",
      });
    }
  ...
  useEffect(() => {
    ...
    const network =
      ...
      (() => {
        ...
        if (sel.includes("arbitrum") && !sel.includes("sepolia"))
          return "arbitrumMainnet";
        if (sel.includes("optimism") && !sel.includes("sepolia"))
          return "optimismMainnet";
        if (sel.includes("base") && !sel.includes("sepolia"))
          return "baseMainnet";
        if (sel.includes("polygon") && !sel.includes("amoy"))
          return "polygonMainnet";
        return "ethereumMainnet";
      })();
    const hasDiscoveredTokens =
      Array.isArray(data.discoveredTokens) && data.discoveredTokens.length > 0;
    const req = hasDiscoveredTokens
      ...
      : fetch(
          `/api/cre/portfolio?address=${encodeURIComponent(data.address)}&network=${encodeURIComponent(network)}`,
        );
    req
      ...
      .then((json: any) => {
        if (json && (json.tvl != null || json.price != null)) {
          ...
          const with0x = data.address.startsWith("0x")
            ? data.address
            : `0x${data.address}`;
  ...
  const applyAnalysisResult = (
    ...
    const latestScanFromAnalysis = {
      ...
      affectedMetrics: result.creObservations?.metrics
        ? Object.keys(result.creObservations.metrics)
        : undefined,
    ...
    const isGenericName =
      !discoveredName ||
      discoveredName === "Discovered Contract" ||
      discoveredName === "New Contract";
    ...
    ContractStorage.updateContract(with0x, {
      ...
      riskLevel: (
        result.creObservations?.riskLevel || "LOW"
      ).toLowerCase() as any,
      status: result.creObservations?.riskLevel || "LOW",
      riskScore: result.creObservations?.riskScore,
      metrics: result.creObservations?.metrics
        ? { ...data?.metrics, ...result.creObservations.metrics }
        : undefined,
    ...
    if (tokensFromAnalysis && tokensFromAnalysis.length > 0) {
      ...
      fetch("/api/cre/portfolio", {
        ...
        .then((portfolioJson: any) => {
          if (
            portfolioJson &&
            (portfolioJson.tvl != null || portfolioJson.price != null)
          ) {
            const metricsUpdate = {
              ...
              ...(portfolioJson.volume24h != null && {
                volume24h: portfolioJson.volume24h,
              }),
              ...(portfolioJson.liquidity != null && {
                liquidity: portfolioJson.liquidity,
              }),
            };
            ContractStorage.updateContract(with0x, {
            ...
            setData((prev: any) =>
              prev
                ? { ...prev, metrics: { ...prev.metrics, ...metricsUpdate } }
                : prev,
            );
            setLiveMetrics({
              tvl: portfolioJson.tvl,
  ...
  const handleFullAnalysis = async () => {
    ...
    setAnalysisLogLines([
      "Starting full analysis (on-chain CRE)…",
      "Requesting on-chain CRE…",
    ]);
    ...
    requestCREAnalysis(data.address, chainSelectorName)
      .then(() => {
        ...
        setAnalysisLogLines((prev) => [
          ...prev,
          "On-chain CRE requested. Waiting for result… (usually 1–2 min)",
        ]);
      ...
      .catch((e: any) => {
        ...
        setAnalysisLogLines((prev) => [
          ...prev.filter((l) => !l.startsWith("Error:")),
          `Error: ${errMsg}`,
        ]);
        toast.warning("Switch network to run CRE", {
          description:
            "Connect to the CRE consumer chain (e.g. Sepolia) and run Full Analysis again.",
        });
  ...
  const contractData = {
    ...
    volume24h:
      rawVolume != null && Number(rawVolume) > 0
        ...
        : data.volume24h && data.volume24h !== "$0.0M"
          ? data.volume24h
          : "$0.00",
    liquidity:
  ...
  const currentVolatility =
    typeof metrics?.volatility === "number"
      ...
      : typeof data.metrics?.volatility === "number"
        ? data.metrics.volatility <= 1
          ? data.metrics.volatility * 100
    ...
    data.history?.volatility?.length > 0
      ...
      : currentVolatility != null
        ? ["6d", "5d", "4d", "3d", "2d", "1d", "Now"].map((time, i) => ({
            ...
            value: Math.max(
              0,
              currentVolatility * (0.85 + (0.15 * (7 - i)) / 7),
            ),
          }))
        : [
  ...
  const riskBreakdown = [
    {
      ...
      value:
        metrics?.volatility != null
          ? Math.round(
              Number(metrics.volatility) * (metrics.volatility <= 1 ? 100 : 1),
            )
          : data.riskScore > 30
            ? Math.min(100, data.riskScore)
            : 23,
    ...
    },
    {
      name: "Liquidity",
      value:
        metrics?.liquidity != null || metrics?.totalLiquidity != null
          ? Math.round(
              metrics?.liquidity ??
                (metrics?.totalLiquidity > 1
                  ? metrics.totalLiquidity * 100
                  : (metrics?.totalLiquidity ?? 0.2) * 100),
            )
          : 20,
    ...
    },
    {
      name: "Manipulation",
      value:
        (metrics as any)?.manipulation != null
          ? Math.round((metrics as any).manipulation * 100)
          : 15,
    ...
    },
    {
      name: "Depeg",
      value:
        metrics?.priceDeviationFromPeg != null
          ? Math.round(
              Number(metrics.priceDeviationFromPeg) *
                (metrics.priceDeviationFromPeg <= 1 ? 100 : 1),
            )
          : data.riskScore > 50
            ? 35
            : 5,
  ...
  const singleVolatilitySeries =
    currentVolatility != null && Number.isFinite(currentVolatility)
      ? [{ name: data.name, volatility: currentVolatility }]
      : [];
  const tvlNumber =
    typeof metrics?.tvl === "number" && Number.isFinite(metrics.tvl)
      ? metrics.tvl
      : typeof metrics?.totalValueLocked === "number" &&
          Number.isFinite(metrics.totalValueLocked)
        ? metrics.totalValueLocked
        : 0;
  const singleLiquiditySeries =
    tvlNumber > 0 ? [{ name: data.name, tvl: tvlNumber }] : [];
              ...
              >
                    ...
                    <div className="rounded-2xl border border-border/50 bg-muted/30 p-4 space-y-2">
                      ...
                      <div className="grid gap-2 text-sm">
                        ...
                        >
                          ...
                          <span>
                            2. CRE workflow picks up request (event on-chain)
                          </span>
                        </div>
                        <div
                          className={cn(
                        ...
                        >
                          ...
                          <div className="min-w-0">
                            <span>
                              3. Risk engine runs (data + checks → report)
                            </span>
                            {lastRequestId && !creAssessment && (
                              <>
                                <p className="text-[11px] text-muted-foreground mt-1.5 font-normal">
                                  CRE workflow simulation (step updates every
                                  ~10s):
                                </p>
                                ...
                                {creWaitingStartTime != null && (
                                  <p className="text-[11px] text-muted-foreground mt-2 font-mono">
                                    Elapsed:{" "}
                                    {Math.floor(creWaitingElapsedSec / 60)}m{" "}
                                    {creWaitingElapsedSec % 60}s
                                  </p>
                                )}
                              </>
                            )}
                      ...
                      {creTxHash && (
                        <a
                          href={
                            CRE_CONSUMER_CHAIN_ID === 11155111
                              ? `https://sepolia.etherscan.io/tx/${creTxHash}`
                              : `https://etherscan.io/tx/${creTxHash}`
                          }
                          target="_blank"
                  ...
                  <div className="rounded-2xl border border-border/50 bg-black/40 font-mono text-sm text-foreground/90 overflow-hidden">
                    ...
                    >
                      {analysisLogLines.map((line, i) => (
                        ...
                        >
                          <span className="text-muted-foreground/70 select-none mr-2">
                            $
                          </span>
                          {line}
                        </motion.div>
                ...
                {analysisNeedMoreInfo && (
                  <div className="px-8 pb-6">
                    <form
                      onSubmit={handleNeedMoreInfoSubmit}
                      ...
                      {analysisNeedMoreInfo.questions.map((q) => (
                        <div key={q.id} className="space-y-1.5">
                          ...
                          <Input
                            ...
                            onChange={(e) =>
                              setNeedMoreInfoFormValues((prev) => ({
                                ...prev,
                                [q.id]: e.target.value,
                              }))
                            }
      ...
      <div className="mx-auto w-full space-y-8 p-6 lg:p-10">
        ...
        <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
          ...
          >
            ...
            {(() => {
              ...
              const blockscoutUrl =
                chain && addr ? getBlockscoutAddressUrl(chain, addr) : null;
        ...
        {(() => {
          ...
          const mitigationStrategy =
            scan?.mitigationStrategy ??
            (fa?.recommendations?.length
              ? fa.recommendations.join("\n\n")
              : undefined);
          const nextSteps = fa?.nextSteps ?? scan?.nextSteps;
          const suggestedActions =
            fa?.suggestedActions ?? scan?.suggestedActions;
                ...
                <div className="space-y-6">
                  ...
                  <div className="space-y-3">
                    ...
                    <p className="text-sm font-medium leading-relaxed text-foreground/90 bg-background/30 p-4 rounded-2xl border border-primary/10 whitespace-pre-wrap">
                      {reasoning ||
                        "Run Full Analysis or Force Scan for an AI summary."}
                    </p>
                  </div>
                  ...
                  <Accordion
                    ...
                    defaultValue={["root-cause", "mitigation"]}
                  >
                    <AccordionItem
                      value="root-cause"
                      className="border border-border/40 rounded-2xl px-4"
                    >
                        ...
                        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
                          <div className="space-y-3">
                            ...
                            <p className="text-xs font-semibold leading-relaxed text-muted-foreground p-4 rounded-2xl border border-rose-500/10 bg-rose-500/[0.02] whitespace-pre-wrap">
                              {cause ||
                                "Run Full Analysis or Force Scan (with post-CRE AI) for root cause analysis."}
                            </p>
                          </div>
                          <div className="space-y-3">
                            ...
                            <p className="text-xs font-semibold leading-relaxed text-muted-foreground p-4 rounded-2xl border border-amber-500/10 bg-amber-500/[0.02] whitespace-pre-wrap">
                              {consequences ||
                                "Run Full Analysis or Force Scan for impact assessment."}
                            </p>
                          </div>
                        </div>
                      </AccordionContent>
                    ...
                    {scan?.affectedMetrics &&
                      scan.affectedMetrics.length > 0 && (
                        <AccordionItem
                          ...
                          <ul className="space-y-2">
                            {suggestedActions.map(
                              (action: string, i: number) => (
                                <li
                                  key={i}
                                  className="flex items-start gap-2 text-xs font-semibold text-foreground/85"
                                >
          ...
          <Card className="overflow-hidden rounded-[2.5rem] border-border/40 bg-card/20 backdrop-blur-xl">
            ...
            <CardContent className="p-8 space-y-8">
              <Accordion type="multiple" className="w-full">
                <AccordionItem
                  value="pre-cre"
                  className="border border-border/40 rounded-2xl px-4 mb-4"
                >
                    ...
                    {analyzeResult.initialAnalysis?.recommendations?.length ? (
                      <div>
                        ...
                        <ul className="list-disc pl-5 mt-1">
                          {analyzeResult.initialAnalysis.recommendations.map(
                            (rec, i) => (
                              <li key={i}>{rec}</li>
                            ),
                          )}
                ...
                >
                  ...
                  <AccordionContent className="pb-4 text-sm text-muted-foreground space-y-3">
                    ...
                    {analyzeResult.finalAnalysis?.recommendations?.length ? (
                      <ul className="list-disc pl-5 space-y-1">
                        {analyzeResult.finalAnalysis.recommendations.map(
                          (r, i) => (
                            <li key={i}>{r}</li>
                          ),
                        )}
